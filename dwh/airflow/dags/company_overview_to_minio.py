...
@dag(
    dag_id="company_overview_to_minio",
...
def company_overview_dag():
    @task
    def log_batches_info(batches: list[dict]):
        logger = logging.getLogger("airflow.task")
        logger.info("Overview run bắt đầu với %d batch", len(batches))
    ...
    ingest_overview = DfToCsvOperator.partial(
        ...
        object_path="overview/date={{ ds }}/batch_{{ ti.map_index }}.csv",
    ...
    def verify_upload(batches: list[dict], ds: str):
        ...
        for idx, _ in enumerate(batches):
            key = f"overview/date={ds}/batch_{idx}.csv"
    ...
    chain(batches, batches_logged, ingest_overview, verify_task)


company_overview_dag()
